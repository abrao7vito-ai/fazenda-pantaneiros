    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white border border-stone-200 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl">
        ...
        <form onSubmit={handleSave} className="p-6 space-y-5">
          ...
          {/* Company Selector Tabs (Only Master holding admin can configure other companies) */}
          {isMaster && companies && companies.length > 1 && (
          ...
          {/* Webhook URL Input - Masked with Security Toggle */}
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-stone-50 border border-stone-300 rounded-xl pl-3.5 pr-10 py-2.5 text-xs text-stone-900 focus:bg-white focus:border-[#5865F2] outline-none font-mono transition-colors"
              />
              <button
                type="button"
                onClick={() => setShowWebhook((v) => !v)}
                className="absolute right-2.5 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-700 p-1"
                title={showWebhook ? "Ocultar URL por segurança" : "Visualizar URL"}
              >
            ...
            <p className="text-[10px] text-stone-400 mt-1 flex items-center gap-1">
              ...
              <span>URL mascarada por segurança para evitar vazamento em compartilhamentos de tela ou transmissões.</span>
            </p>
          </div>
